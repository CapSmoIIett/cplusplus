...
std::weak_ptr - не является автономным интеллектуальным указателем, это дополнение к std::shared_ptr. 
...
чтобы из std::weak_ptr получить данные из него нужно сделать std::shared_ptr
